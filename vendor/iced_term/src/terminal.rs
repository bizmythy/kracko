...
impl Terminal {
    ...
    pub fn handle(&mut self, cmd: Command) -> Action {
        let _span = crate::prof::Span::start(format!(
            ...
            self.id,
            match &cmd {
                Command::ChangeTheme(_) => "ChangeTheme".to_string(),
                Command::ChangeFont(_) => "ChangeFont".to_string(),
                Command::AddBindings(_) => "AddBindings".to_string(),
                Command::ProxyToBackend(cmd) => format!("Backend({cmd:?})"),
            }
        ));
