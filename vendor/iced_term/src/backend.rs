...
impl Backend {
    ...
    fn resize(
        ...
        let cols = (self.size.layout_width / self.size.cell_width as f32)
            .floor() as u16;
        // Only disturb the PTY and the grid when the dimensions actually
        // change; pixel-level layout changes (e.g. live divider drags)
        // otherwise trigger a resize/redraw storm in the shell.
        if lines > 0
            && cols > 0
