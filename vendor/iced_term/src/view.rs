    ...
    fn handle_resize(
        ...
        if state.size != layout_size {
            crate::prof::mark(format!(
                "publish resize term {}: {:?} -> {:?}",
                self.term.id, state.size, layout_size
            ));
...
impl Widget<Event, Theme, iced::Renderer> for TerminalView<'_> {
    ...
    fn draw(
        ...
        let _span = crate::prof::Span::start(format!(
            "draw term {} (size {:?})",
            self.term.id,
            bounds.size()
        ));

        // The geometry is cached in the widget's local coordinate space and
        // translated into place when drawn. Baking the layout position into
        // the cache (as upstream does) renders stale positions whenever the
        // widget moves without its content changing — e.g. inside a
        // scrollable, or when a sibling pane is resized or inserted. Keying
        // the cache on the widget size (not the clip viewport size) also
        // avoids re-shaping the whole grid every frame while the widget is
        // only partially visible.
        let geom = self.term.cache.draw(renderer, bounds.size(), |frame| {
            let _span = crate::prof::Span::start(format!(
                "  re-record geometry term {}",
                self.term.id
            ));
            ...
            for indexed in content.grid.display_iter() {
                ...
                // Resolve position point for this cell, in local coordinates
        ...
        renderer.with_translation(
            bounds.position() - iced::Point::ORIGIN,
            |renderer| renderer.draw_geometry(geom),
        );
    }
